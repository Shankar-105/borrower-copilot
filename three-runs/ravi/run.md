# Ravi — run-through

## Profile

- Age: 42
- City: Mysuru
- Income: self-employed kirana store owner
- Cash income: ₹40,000–₹80,000/month
- Documented ITR income: ₹4,20,000/year
- Existing EMI: ₹0
- Credit score: Not known
- Purpose: second stock line and delivery vehicle
- Loan type: business loan
- Requested amount: ₹15,00,000
- Tenure: 60 months
- Recent bounce: No
- High-cost debt above 24%: No
- Household expenses: Not known from the challenge profile
- Unencumbered shop value: ₹45,00,000

The app uses the ITR income for the main calculation because it is documented. It does not use the ₹40k–₹80k cash range as if all of it is stable income.

## Questions shown

1. What are you borrowing for? — Business or stock
2. What loan type are you considering? — Business loan
3. How much do you want to borrow? — ₹15,00,000
4. How does your income arrive? — Self-employed
5. Lower monthly income — ₹40,000
6. Higher monthly income — ₹80,000
7. Annual documented income (ITR) — ₹4,20,000
8. Existing monthly EMIs — ₹0
9. Do you know your monthly household expenses? — Not yet
10. Your age — 42
11. Credit score, if known — Not known
12. Unencumbered property or collateral value — ₹45,00,000
13. Any EMI bounced in the last 6 months? — No
14. Any app or short-term debt above 24%? — No
15. Preferred tenure — 5 years

## O1 — Should I borrow?

**BORROW LESS**

The ₹15L request is much higher than the borrower-safe amount of about ₹5.8L. The property gives a secured route, but collateral does not replace income affordability.

Confidence: **Low** because the credit score and household expenses are not known, and the income is self-employed.

## O2 — How much?

- Estimated lender-side capacity: **about ₹7.3L**
- Income-based borrower-safe amount: **about ₹5.8L**
- Collateral cap at 50% LTV: **₹22.5L**
- Final borrower-safe amount: **about ₹5.8L**
- Safe EMI ceiling: **₹14,000/month**

The borrower should use the **₹5.8L borrower-safe amount**. The ₹22.5L collateral cap is only an upper limit. Income affordability is the lower limit here.

## O3 — What rate?

- Route: **Loan against property / business loan**
- Fair rate band: **13%–18%**
- All-in APR estimate: **about 14.8%–20.7%**
- Prototype processing fee: **2%**, about ₹11,641 on the safe amount

The secured route starts from the 10%–14% prototype band. Unknown credit adds 2 percentage points and widens the upper end. Non-salaried income adds another 1 percentage point.

## O4 — What EMI?

- Base safe EMI ceiling: **₹14,000/month**
- Tenure used: **60 months**
- Tenure trade-off on the safe amount at the 15.5% midpoint:
  - 36 months: about **₹20.3k EMI**, about **₹1.49L total interest**
  - 48 months: about **₹16.3k EMI**, about **₹2.03L total interest**
  - 60 months: about **₹14.0k EMI**, about **₹2.58L total interest** — selected
- Stress income after a 15% drop: **₹29,750/month**
- Stressed safe EMI room: **₹11,900/month**
- Requested ₹15L EMI at the stressed rate of 20%: **about ₹39,741/month**
- Stress result: **Buffer breaks**

The trade-off shows why the selected 60 months fits the safe EMI ceiling better, but also costs more total interest than shorter terms.

The requested amount is far above the safe range, so the borrower should reduce the request before going to the lender.

## Negotiation Card

The card should show:

- Recommended amount: **about ₹5.8L**
- Fair rate: **13%–18%**
- APR including fee: **about 14.8%–20.7%**
- EMI ceiling: **₹14,000/month**
- Route: **Loan against property / business loan**
- Verdict reason: the ₹15L request is above the borrower-safe amount

I will add the result/card screenshots in this folder.
