# Priya — run-through

## Profile

- Age: 29
- City: Bengaluru
- Income: salaried
- Net monthly income: ₹1,10,000
- Existing EMI: ₹14,000 for car loan
- Credit score: 780
- Purpose: wedding
- Loan type: personal loan
- Requested amount: ₹8,00,000
- Tenure: 48 months
- Recent bounce: No
- High-cost debt above 24%: No
- Household expenses: Not fully known from the challenge profile
- Collateral: Not supplied

I did not enter the ₹28,000 rent as total household expenses because the challenge gives rent, not the full monthly household expense number. The app keeps the expense value unknown instead of making up a total.

## Questions shown

1. What are you borrowing for? — Wedding or family event
2. What loan type are you considering? — Personal loan
3. How much do you want to borrow? — ₹8,00,000
4. How does your income arrive? — Salaried
5. Your usual monthly take-home — ₹1,10,000
6. Existing monthly EMIs — ₹14,000
7. Do you know your monthly household expenses? — Not yet
8. Your age — 29
9. Credit score, if known — 780
10. Any EMI bounced in the last 6 months? — No
11. Any app or short-term debt above 24%? — No
12. Preferred tenure — 4 years

The collateral question is not shown because this is not a business/self-employed/LAP case.

## O1 — Should I borrow?

**BORROW**

The ₹8L request is below the borrower-safe amount of about ₹11.2L. The current profile has a strong stated credit score and no recent bounce or high-cost debt.

Confidence: **Medium** because total household expenses are not known.

## O2 — How much?

- Estimated lender-side capacity: **about ₹15.3L**
- Borrower-safe amount: **about ₹11.2L**
- New safe EMI ceiling: **₹30,000/month**

The borrower should use the **₹11.2L borrower-safe amount**, not the lender-side estimate.

The lender-side number uses 50% FOIR. The safe number uses 40% FOIR and subtracts the existing ₹14,000 EMI first.

## O3 — What rate?

- Fair rate band: **9.5%–16.5%**
- All-in APR estimate: **about 11.1%–19.1%**
- Prototype processing fee: **2%**, about ₹22,365 on the safe amount

The 780 score moves the personal-loan reference band down by 1.5 percentage points. The app still shows a range because the actual lender quote is not known.

## O4 — What EMI?

- Base safe EMI ceiling: **₹30,000/month**
- Tenure used: **48 months**
- Stress income after a 15% drop: **₹93,500/month**
- Stressed safe EMI room: **₹23,400/month**
- Requested ₹8L EMI at the stressed rate of 18.5%: **about ₹23,710/month**
- Stress result: **Buffer breaks slightly**

The base verdict is still BORROW because the stated request fits the current safe amount. The stress result tells Priya that ₹8L is close to the limit under a weaker income/higher-rate case.

## Negotiation Card

The card should show:

- Recommended amount: **about ₹11.2L**
- Fair rate: **9.5%–16.5%**
- APR including fee: **about 11.1%–19.1%**
- EMI ceiling: **₹30,000/month**
- Route: **Personal loan**
- Verdict reason: the ₹8L request is below the borrower-safe amount

I will add the result/card screenshots in this folder.
