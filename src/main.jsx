...
function App() {
  ...
  return <div className="app-shell">
    ...
    {stage === 'form' && <main className="workspace"><div className="progress"><span>YOUR PICTURE</span><div><i className="active"/><i/><i/></div><span>1 / 3</span></div><div className="workspace-heading"><div className="kicker">Start with the minimum picture</div><h1>What would make this borrowing <em>wise?</em></h1><p>The first questions are enough to produce all four outputs. Extra questions are optional and only appear when they can tighten an answer. Unknown information stays unknown; it never quietly becomes zero.</p></div><div className="form-layout"><section className="question-panel"><div className="section-label">Must answer</div><div className="question-grid">{mustQuestions.map((question) => <QuestionField key={question.id} question={question} profile={profile} update={update} />)}</div>{additionalQuestions.length > 0 && <details className="additional-questions"><summary>Improve the estimate with more details</summary><p>Each of these changes a number, the route, the decision, or the confidence. You can leave them unanswered.</p><div className="question-grid">{additionalQuestions.map((question) => <QuestionField key={question.id} question={question} profile={profile} update={update} />)}</div></details>}<div className="form-actions"><button className="quiet-button" onClick={reset}>Start over</button><button className="primary-button" disabled={!rentIsValid} aria-disabled={!rentIsValid} onClick={() => setStage('result')}>See my assessment <span>↗</span></button></div></section><aside className="live-preview"><div className="section-label">Live preview</div><div className="preview-number">{formatLakhs(result.safeAmount)}</div><p>borrower-safe amount to plan around</p><div className="preview-line"><span>Borrower-safe EMI ceiling</span><b>{formatInr(result.recommendedEmi)}</b></div><div className="preview-line"><span>Borrower-safe amount</span><b>{formatLakhs(result.safeAmount)}</b></div><div className="preview-line"><span>Confidence</span><b>{result.confidence.level}</b></div><div className="preview-note">Updates as you answer. This is a planning estimate, not an approval.</div></aside></div></main>}
