...
function QuestionField({ question, profile, update }) {
  ...
  const showMissing = missing && (profile[question.id] != null || (question.id === 'monthlyRent' && profile.housingType === 'rent'))
  return <label className="field" key={question.id}><span>{question.label}{question.optional && <small> optional</small>}{question.required && <small> required</small>}</span>{question.type === 'select' ? <select value={String(profile[question.id] ?? '')} onChange={(event) => update(question.id, event.target.value)}>{question.options.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select> : <div className="input-wrap">{question.prefix && <b>{question.prefix}</b>}<input type="number" min={question.min ?? 0} max={question.max} required={question.required} aria-invalid={showMissing} value={profile[question.id] ?? ''} onChange={(event) => update(question.id, event.target.value)} />{question.suffix && <b>{question.suffix}</b>}</div>}{showMissing && <small className="field-error">Enter this value to continue.</small>}<small className="affects">Affects {question.affects}</small></label>
}
...
function App() {
  ...
  let rentIsValid = profile.housingType !== 'rent' || Number(profile.monthlyRent) > 0
  ...
  rentIsValid = rentIsValid && mustQuestionsAreComplete
